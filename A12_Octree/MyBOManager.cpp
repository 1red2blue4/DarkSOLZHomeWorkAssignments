...
void MyBOManager::CheckCollisions(void)
{
	...
	*/
	...
	{
		m_lObject[i]->SetColor(REGREEN);
	}

	for (int nObjectA = 0; nObjectA < m_nObjectCount - 1; nObjectA++)
	{
		for (int nObjectB = 0; nObjectB < m_nObjectCount; nObjectB++)
		{
			if (m_lObject[nObjectA]->IsColliding(m_lObject[nObjectB]))
			{
				m_lObject[nObjectA]->SetColor(RERED);
				m_lObject[nObjectB]->SetColor(RERED);
			}
		}
	}
}
